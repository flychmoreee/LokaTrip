            ...
            @if ($isDelete)
                ...
                <form action="{{ $action }}" method="POST">
                    ...
                    <div class="modal-footer justify-content-center">
                        ...
                        <button type="submit" class="btn btn-danger color-fff">Hapus</button>
                    </div>
                </form>
            @else
                        ...
                        @foreach ($fields as $field)
                            <div class="mb-3">
                                ...
                                @elseif($field['type'] === 'textarea')
                                    <textarea class="form-control" id="{{ $field['name'] }}" name="{{ $field['name'] }}" rows="{{ $field['rows'] ?? 3 }}"
                                        placeholder="{{ $field['placeholder'] ?? '' }}"
                                        {{ isset($field['required']) && $field['required'] ? 'required' : '' }}>{{ $field['value'] ?? '' }}</textarea>
                                @elseif($field['type'] === 'time')
                                    <input type="time" class="form-control" id="{{ $field['name'] }}"
                                        name="{{ $field['name'] }}" value="{{ $field['value'] ?? '' }}"
                                        {{ isset($field['required']) && $field['required'] ? 'required' : '' }}>
                                @elseif($field['type'] === 'file')
                                    <input type="file" class="form-control" id="{{ $field['name'] }}"
                                        name="{{ $field['name'] }}" accept="{{ $field['accept'] ?? 'image/*' }}"
